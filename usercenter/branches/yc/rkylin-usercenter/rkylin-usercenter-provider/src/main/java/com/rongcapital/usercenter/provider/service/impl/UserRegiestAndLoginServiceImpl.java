...
import com.rongcapital.usercenter.provider.constant.SystemConstants;
...
public class UserRegiestAndLoginServiceImpl implements UserRegiestAndLoginService {
    ...
    public static final String WECHATAPPCODE = "38";
  
    public static final int WECHAT_REGIST_TYPE = 3;//表微信注册
...
//                                    password, regiesterInfo.getVcode(), null);
                    ...
                    BaseResultVo<RegiesterResponse> regiest = new   BaseResultVo<RegiesterResponse>();
                    if (true) {
                      ...
                      //  localUserService.modifyCaasUserCode(userInfo.getUserInfoId(), caasUserCode.getUserCode());
                        ...
                        regiesterInfo.setPassword(password);//加密后的串打印日志
                ...
                if (regiest.isSuccess()) {
                    ...
                    regiesterInfo.setPassword(password);//加密后的串
...
//                            xAuthToken);
            BaseResultVo<LoginResponse> login =new   BaseResultVo<LoginResponse>();
...
//                        this.caasUserServiceProxy.userAuth(configfile, appCode, loginResponse.getAuthCode());
                BaseResultVo<AuthResultVo> userAuth =  new BaseResultVo<AuthResultVo>();
               // AuthResultVo authResultVo = userAuth.getResultData();
                AuthResultVo authResultVo = new AuthResultVo();
                authResultVo.setAccessToken("dafdfa");
                authResultVo.setExpiresIn(360000);
...
//                String userToken = UUID.randomUUID().toString();
              //  if (userAuth.isSuccess()) {
                    ...
                    try {
                        ...
                        UserRedisSessionUtils.setUserNameAndUserToken(loginName, orgCode, userToken,300000);
                       //缓存caas check结果 登陆成功并且授权成功,认为check成功
                        UserRedisSessionUtils.setCaasCheckResult(userToken,Integer.parseInt("3600"));
                    } catch (Exception e) {
                        LOGGER.error("归零redis失败次数失败", e);
                    }

                    String accessToken = "23123123";
                    ...
                    LOGGER.debug("登录接口调用授权接口进行redis记录操作,appCode:{},terminalType:{},authCode:{},accessToken:{}",
                            appCode, terminalType, "dfadf", accessToken);
    ...
    public BaseResultVo<ResponseResultVo> weChatRegist(WeChatRegisterInfo weChatRegiesterInfo)
        ...
        LOGGER.info("weChatRegist入参weChatRegiesterInfo：{}",weChatRegiesterInfo.toString());
      //  String weChatId = weChatRegiesterInfo.getWeChatId();
        String weChatId = SystemConstants.WECHAT_REGIST_PLATFORM;//微信平台
        ...
        String teleNum=weChatRegiesterInfo.getTeleNum();
        String certificateType = weChatRegiesterInfo.getCertificateType();
        String certificateNumber=weChatRegiesterInfo.getCertificateNumber();
        String configfile = "";
        String password =
        ...
        if (StringUtils.isEmpty(certificateType)) {
            resultVo.setErrorMsg("证件类型不能为空");
            resultVo.setResponseCode(emptyErrorCode);
            return resultVo;
        }
        if (StringUtils.isEmpty(certificateNumber)) {
            resultVo.setErrorMsg("证件号码不能为空");
            resultVo.setResponseCode(emptyErrorCode);
            return resultVo;
        }
        ...
        UserInfo userInfo = new UserInfo();
        userInfo.setUserType(WECHAT_REGIST_TYPE);
        userInfo.setStatus(Status.SUCCESS.value()); // 状态
        userInfo.setUserId(userId);
        userInfo.setOrgCode(orgCode);
        userInfo.setUserCode(userCode);
        ...
        userLogin.setUserType(WECHAT_REGIST_TYPE);

        PersonInfo personInfo = new PersonInfo();
        personInfo.setMobilePhone(teleNum);//手机号
        personInfo.setOtherinfo2(certificateType);//证件类型
        personInfo.setIdNumber(certificateNumber);//身份证号
        ...
        try {
            ...
            localUserService.regiestOtherInfo(userInfo, userLogin,personInfo);
        } catch (Exception e) {
